...
function buildPDF(dataCallback, endCallback, data) {
    ...
    doc.font(robotoRegular).fontSize(10).text('Kraków, 27 września 2022', 411, 85, { width: 150, align: 'right' });

    //Title
    doc.font(robotoMedium).fontSize(14).text('DOKUMENT FAKTURY NR: 6316e79989c0a7e461297afd', 100, 120);
    ...
    //Buyer details
    doc.font(robotoMedium).fontSize(10).text('Nabywaca:', 30, 212);
    doc.font(robotoMedium).fontSize(10).text('Igor Kłusek', 30, 224);
    doc.font(robotoMedium).fontSize(10).text('32-828 Biadoliny Szlacheckie, Biadoliny Szlacheckie 142', 30, 236);

    //Sale details
    doc.font(robotoRegular).fontSize(10).text('Data sprzedaży: 21 września 2022', 30, 260);
    doc.font(robotoRegular).fontSize(10).text('Sposób zapłaty: płatność elektroniczna', 30, 272);
    doc.font(robotoRegular).fontSize(10).text('Termin płatności: 26 września 2022', 30, 284);
    ...
    let numberOfUniqueOrderedProducts = 1;
    for (var i = 0; i < numberOfUniqueOrderedProducts; i++) {
        ...
        if (i === numberOfUniqueOrderedProducts - 1) {
            ...
            doc.moveTo(570, 331 + lineHeight + 13)
                ...
                .stroke();
            //---------------Horizontal line
            doc.moveTo(525, 344 + lineHeight + 13)
                .lineTo(570, 344 + lineHeight + 13)
                .stroke();

            doc.font(robotoRegular)
                .fontSize(10)
                .text('Ogółem', 480, 344 + lineHeight + 3);
            tableEndPosition = 344 + lineHeight + 13;
        }
    }
    ...
    doc.font(robotoRegular)
        .fontSize(10)
        .text('72,28', 299, tableEndPosition + 30, { width: 110, align: 'right' });
    doc.font(robotoRegular)
        .fontSize(10)
        .text('16,62', 381, tableEndPosition + 30, { width: 110, align: 'right' });
    doc.font(robotoRegular)
        .fontSize(10)
        .text('88,90', 459, tableEndPosition + 30, { width: 110, align: 'right' });
    //SumUp--------InTotal
    ...
    doc.font(robotoRegular)
        .fontSize(10)
        .text('72,28', 299, tableEndPosition + 42, { width: 110, align: 'right' });
    doc.font(robotoRegular)
        .fontSize(10)
        .text('16,62', 381, tableEndPosition + 42, { width: 110, align: 'right' });
    doc.font(robotoRegular)
        .fontSize(10)
        .text('88,90', 459, tableEndPosition + 42, { width: 110, align: 'right' });
    ...
    doc.font(robotoRegular)
        .fontSize(10)
        .text('88,90 zł: ', 100, tableEndPosition + 70);

    doc.font(robotoMedium)
        .fontSize(12)
        .text(
            'Uwaga: Dokument faktury nie jest dokumentem prawdziwym i nie jest podstawą do odliczenia VAT.',
            100,
            tableEndPosition + 100
        );
